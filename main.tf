...
data "gns3_template_id" "router_id" {
  name = "c3725"  # Replace with your router template's name if different.
}

# Create a switch resource that uses the project_id computed from the project resource
resource "gns3_switch" "switch1" {
  project_id = gns3_project.project1.project_id
  name       = "Switch1"
}

# Create a cloud resource that uses the project_id computed from the project resource
